class AddCertificateArchiving < ActiveRecord::Migration[8.1]
  def change
    ...
    add_index :certificates, [:area, :archived, :active]
    remove_index :certificates, [:area, :source, :source_id], unique: true
    add_index :certificates, [:area, :source, :source_id, :fingerprint], unique: true,
