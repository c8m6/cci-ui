...
test('language switching retains live search filters and POST-rendered previews', { timeout: 60000 }, async () => {
  ...
  try {
    ...
    await page.click('nav a[href="/imports/new"]');
    await page.waitForSelector('textarea[name=pem]');
    await page.click('form[action="/imports"] details summary');
    ...
    await Promise.all([
      page.waitForNavigation({ waitUntil: 'networkidle0' }),
      page.click('form[action="/imports"] input[type=submit]'),
    ]);
    assert.equal(new URL(page.url()).pathname, '/imports');
    const token = await page.$eval('input[name=token]', node => node.value);
    await switchLanguage('en');
    assert.equal(new URL(page.url()).pathname, '/imports/preview');
