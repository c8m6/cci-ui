...
class CertificateProvenanceTest < ActiveSupport::TestCase
  ...
  test "historical versions without provenance remain readable and clear stale metadata" do
    ...
    path = "#{ConsulStore.prefix(record.area)}/versions/#{record.source_id}"
    ...
    assert_equal data["pem"], reader.fetch(area: record.area, lookup: record.lookup)
  end

  test "writers must explicitly identify themselves before any write" do
    cert, = issue
    args = { area: "zone_a", cert: cert, key: nil, chain: [], tags: [], lookup: "missing", actor: "test" }
  ...
  test "standalone Ruby example writes compatible versions keys provenance and audit atomically" do
    cert, key = issue
    args = { area: "zone_a", lookup: "ruby-example", cert: cert, key: key,
    ...
    assert_equal "acme-renewer", reader.fetch(area: "zone_a", lookup: "ruby-example", field: "metadata")["client"]
    assert cert.check_private_key(OpenSSL::PKey.read(reader.fetch(area: "zone_a", lookup: "ruby-example", field: "private_key")))
    assert_equal "service-account", AuditEvent.find_by!(action: "import").actor
    assert_raises(ConsulConnection::Conflict) { CertificateExample.add(**args) }
    ...
    assert Certificate.find_by!(source_id: renewed).active
    assert_equal 2, AuditEvent.where(action: "import").count
  end
end
