...
class AuditEventTest < ActiveSupport::TestCase
  test "Consul mutations preserve snapshots and original event time after deletion and repeated indexing" do
    ...
    travel_to Time.current.change(usec: 0) do
      ...
      travel 2.minutes
      ConsulStore.delete("zone_a", second.source_id, actor: "delete-user")
      CatalogIndexer.refresh_consul
      assert_not Certificate.exists?(second.id)
      ...
      deleted = AuditEvent.find_by!(action: "delete")
      assert_equal "delete-user", deleted.actor
      ...
      assert_no_difference "AuditEvent.count" do
        CatalogIndexer.refresh_consul
        assert_raises(Certificates::Error) { ConsulStore.delete("zone_a", first.source_id, actor: "denied") }
        CatalogIndexer.refresh_consul
      end
    end
