...
class ConsulStore
  ...
  def self.set_status(area, id, status:, actor:, expected_certid_index:)
    ...
    certid, = split_id(id)
    data = get(area, id)
    mutate(area, certid, actor: actor, action: "status_change", id: id, data: data,
      expected_index: expected_certid_index) do |entry|
  ...
  def self.archive(record, actor:, expected_certid_index:)
    ...
    mutate(record.area, record.certid, actor: actor, action: "archive", id: record.source_id, data: data,
      expected_index: expected_certid_index) do |entry|
